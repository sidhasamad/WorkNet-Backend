...
export const getUserJob=async(req,res)=>{
  try{
    const getAllJob= await jobPosts.find().populate({
      path:'employer',
      model:'Employer',
       select: 'Logo'
    })

    res.status(200).json({success:true,message:'Get all jobs',data:getAllJob})
  }catch(error){
...
export const getUserJobId=async(req,res)=>{
  try{
    ...
    res
    .status(200)
    .json({success:true,message:'Job Details',data:job})
  }catch(error){
    res 
    .status(500)
